...
TimescaleDB runs behind the `timescale` profile with a pinned
`timescale/timescaledb:2.15.3-pg16` image. The main `pg` service remains plain
PostgreSQL; Timescale uses its own init directory because it requires the
TimescaleDB extension and preload behavior:

```sh
docker compose -f docker/docker-compose.yml --profile timescale up -d timescale
psql "postgresql://pgfound:pgfound@localhost:5438/pgfound" \
