# Decision Report: healthcare-ops-minimal
...
### `postgres_fdw` (extension, recommend_now, 0.80)

Why now:
- The intake names remote PostgreSQL access and an FDW federation need.
- postgres_fdw is the narrowest PostgreSQL-native bridge when remote paths are bounded.

Why not yet:
- FDW should not become a hidden permanent hot path without pushdown verification and retirement planning.

Next-stage triggers:
- Require EXPLAIN VERBOSE pushdown checks and a materialization plan for hot remote reads.

Sources:
- `rule-postgres-fdw-for-federation` (0.80)
...
### `postgres_fdw_federation` (topology_pattern, recommend_now, 0.76)

Why now:
- A federation topology makes ownership and remote failure modes explicit.
- Migration-bridge workloads need explicit remote-source boundaries and owner-visible failure modes.

Why not yet:
- Avoid broad cross-database joins until latency and remote-owner behavior are understood.
- Avoid broad permanent federation without pushdown checks and decommission criteria.

Next-stage triggers:
- Document credentials, remote health monitoring, and fallback behavior.
- Adopt once remote credentials, latency SLOs, and fallback behavior are documented.

Sources:
- `rule-postgres-fdw-for-federation` (0.76)
- `rule-fdw-federation-for-modernization-bridge` (0.74)
...
### `no_restore_drills` (anti_pattern_warning, avoid_for_now, 0.68)

Why now:
- Low operational tolerance plus complex data or bridge needs requires restore proof before adopting more moving parts.

Why not yet:
- Simple core features can still proceed with modest runbooks.

Next-stage triggers:
- Require restore drills for extension objects, remote definitions, partition metadata, and large indexes.

Sources:
- `rule-warn-no-restore-drills` (0.68)
