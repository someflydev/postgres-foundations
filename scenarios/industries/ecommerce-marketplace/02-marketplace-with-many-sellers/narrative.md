# Marketplace with sellers as tenants

Marketplace with sellers as tenants is a mid-stage pressure judgment brief from a team
of 38 engineers working in ecommerce marketplace. The product serves operators who live
in PostgreSQL-backed workflows every day: support staff, finance reviewers, compliance
analysts, and customer-facing teams who need answers while the business is open. The
current system runs on primary with read replicas with PostgreSQL as the operational
source of truth. Largest tables include orders (76,000,000 rows), inventory_movements
(190,000,000 rows), seller_events (130,000,000 rows). Peak traffic is roughly 5400 read
queries per second, 980 writes per second, and 980 database connections during busy
windows.
...
The tenancy model is multi tenant shared schema. The data shape mix is relational_core,
semi_structured_jsonb, append_only_events, and the workload patterns are oltp_heavy,
read_heavy, append_heavy, strong_tenant_locality, analytics_adjacent. The growth horizon
is concrete: in 6 months the team expects to absorb the next customer or seasonal cycle,
in 12 months they expect current hot tables to roughly follow 15 percent month-over-
month growth unless product scope narrows, and in 24 months they expect today's
borderline decisions to become operational constraints. The known pending decision is
this: Sellers act like tenants, and the team needs a decision on partitioning orders and
inventory movement history before holiday volume.

The scenario is not a textbook case. Some sellers share fulfillment networks, so tenant
locality is strong for dashboards but weaker for shipment allocation. The team has
