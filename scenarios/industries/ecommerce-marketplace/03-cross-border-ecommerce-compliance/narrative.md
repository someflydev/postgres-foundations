# Cross-border ecommerce compliance platform

Cross-border ecommerce compliance platform is a high-scale mature brief from a team of
69 engineers working in ecommerce marketplace. The product serves operators who live in
...
source of truth. Largest tables include orders (180,000,000 rows), tax_events
(360,000,000 rows), compliance_documents (42,000,000 rows). Peak traffic is roughly 9100
read queries per second, 1900 writes per second, and 1700 database connections during
busy windows.
...
append_only_events, full_text_docs, foreign_postgres_access, and the workload patterns
are oltp_heavy, append_heavy, read_heavy, search_heavy, migration_bridge,
analytics_adjacent. The growth horizon is concrete: in 6 months the team expects to
absorb the next customer or seasonal cycle, in 12 months they expect current hot tables
to roughly follow 14 percent month-over-month growth unless product scope narrows, and
in 24 months they expect today's borderline decisions to become operational constraints.
The known pending decision is this: They need logical replication for a compliance
datastore, FDW during regional migration, and search over customs documents without
surrendering portability.

The scenario is not a textbook case. Cross-border refunds can mutate compliance state
weeks later, making append-only audit records necessary but not sufficient for current-
state queries. The team has enough PostgreSQL skill to use indexes, constraints, RLS,
