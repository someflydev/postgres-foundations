# Startup payments gateway before formal ledger scale

Startup payments gateway before formal ledger scale is a small early conservative brief
from a team of 10 engineers working in fintech payments. The product serves operators
who live in PostgreSQL-backed workflows every day: support staff, finance reviewers,
compliance analysts, and customer-facing teams who need answers while the business is
open. The current system runs on single primary with PostgreSQL as the operational
source of truth. Largest tables include payments (850,000 rows), ledger_entries
(2,800,000 rows), webhook_events (4,100,000 rows). Peak traffic is roughly 360 read
queries per second, 95 writes per second, and 120 database connections during busy
windows.
...
append_only_events, and the workload patterns are oltp_heavy, append_heavy. The growth
horizon is concrete: in 6 months the team expects to absorb the next customer or
seasonal cycle, in 12 months they expect current hot tables to roughly follow 13 percent
...
decision is this: They want immutable ledger checks and better idempotency before any
extension discussion; pgcrypto is being considered for public payment references.

The scenario is not a textbook case. Reconciliation is still partly spreadsheet-driven,
so operational risk comes from human correction loops as much as query speed. The team
has enough PostgreSQL skill to use indexes, constraints, RLS, replication, and managed
