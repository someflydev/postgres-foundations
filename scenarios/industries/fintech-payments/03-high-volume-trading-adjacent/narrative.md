# High-volume trading-adjacent payments reconciliation

High-volume trading-adjacent payments reconciliation is a high-scale mature brief from a
team of 72 engineers working in fintech payments. The product serves operators who live
...
source of truth. Largest tables include fills (210,000,000 rows), ledger_entries
(410,000,000 rows), risk_snapshots (98,000,000 rows). Peak traffic is roughly 11200 read
queries per second, 2400 writes per second, and 1450 database connections during busy
windows.
...
append_only_events, time_series_metrics, and the workload patterns are oltp_heavy,
append_heavy, analytics_adjacent, read_heavy. The growth horizon is concrete: in 6
months the team expects to absorb the next customer or seasonal cycle, in 12 months they
expect current hot tables to roughly follow 18 percent month-over-month growth unless
product scope narrows, and in 24 months they expect today's borderline decisions to
become operational constraints. The known pending decision is this: They claim a strong
RPO target but have not documented restore drills for partition metadata, large BRIN
indexes, or audit exports.

The scenario is not a textbook case. The workload is trading-adjacent rather than order
matching, so correctness and reconciliation explainability outrank microsecond latency.
