# Hospital system audit-heavy operations

Hospital system audit-heavy operations is a mid-stage pressure judgment brief from a
team of 41 engineers working in healthcare ops. The product serves operators who live in
...
source of truth. Largest tables include encounters (18,000,000 rows), audit_events
(220,000,000 rows), documents (9,500,000 rows). Peak traffic is roughly 4200 read
queries per second, 850 writes per second, and 760 database connections during busy
windows.
...
append_only_events, full_text_docs, and the workload patterns are oltp_heavy,
append_heavy, search_heavy, analytics_adjacent. The growth horizon is concrete: in 6
months the team expects to absorb the next customer or seasonal cycle, in 12 months they
expect current hot tables to roughly follow 10 percent month-over-month growth unless
product scope narrows, and in 24 months they expect today's borderline decisions to
become operational constraints. The known pending decision is this: They are weighing
native full-text search, pg_trgm for misspelled patient names, and replica isolation for
compliance reporting.

The scenario is not a textbook case. Audit investigators search by vague clinician
notes, but patient-matching false positives can create real compliance work. The team
