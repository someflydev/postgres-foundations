# Clinic operations platform adjacent to EHR

Clinic operations platform adjacent to EHR is a small early conservative brief from a
team of 9 engineers working in healthcare ops. The product serves operators who live in
...
truth. Largest tables include appointments (760,000 rows), patients (210,000 rows),
clinical_notes_metadata (1,200,000 rows). Peak traffic is roughly 480 read queries per
second, 70 writes per second, and 90 database connections during busy windows.
...
ranges_windows, semi_structured_jsonb, and the workload patterns are oltp_heavy,
read_heavy. The growth horizon is concrete: in 6 months the team expects to absorb the
next customer or seasonal cycle, in 12 months they expect current hot tables to roughly
follow 9 percent month-over-month growth unless product scope narrows, and in 24 months
they expect today's borderline decisions to become operational constraints. The known
pending decision is this: They need appointment conflict protection and audit trails
before considering search or analytics extensions.

The scenario is not a textbook case. Some clinics share providers across locations,
making simple office-based scheduling rules fail during holiday coverage. The team has
