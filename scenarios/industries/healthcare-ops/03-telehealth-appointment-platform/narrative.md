# Telehealth appointment platform at national scale

Telehealth appointment platform at national scale is a high-scale mature brief from a
team of 64 engineers working in healthcare ops. The product serves operators who live in
...
source of truth. Largest tables include appointments (88,000,000 rows),
availability_windows (260,000,000 rows), video_session_events (340,000,000 rows). Peak
traffic is roughly 7800 read queries per second, 1700 writes per second, and 2100
database connections during busy windows.
...
ranges_windows, append_only_events, time_series_metrics, and the workload patterns are
oltp_heavy, append_heavy, read_heavy, strong_tenant_locality. The growth horizon is
concrete: in 6 months the team expects to absorb the next customer or seasonal cycle, in
12 months they expect current hot tables to roughly follow 17 percent month-over-month
growth unless product scope narrows, and in 24 months they expect today's borderline
decisions to become operational constraints. The known pending decision is this: They
need tenant isolation, exclusion-style booking protection, and a posture on whether
time-series tooling is justified for video-quality metrics.

The scenario is not a textbook case. Provider groups behave like tenants, but patients
move between groups after insurance changes, so isolation cannot erase cross-group care
workflows. The team has enough PostgreSQL skill to use indexes, constraints, RLS,
replication, and managed extensions, but not enough spare capacity to adopt every
