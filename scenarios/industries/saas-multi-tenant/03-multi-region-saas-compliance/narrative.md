# Multi-region SaaS compliance with one very large tenant

Multi-region SaaS compliance with one very large tenant is a high-scale mature brief
from a team of 58 engineers working in saas multi tenant. The product serves operators
...
operational source of truth. Largest tables include tenant_events (260,000,000 rows),
accounts (31,000,000 rows), permission_audit (90,000,000 rows). Peak traffic is roughly
8600 read queries per second, 1800 writes per second, and 1800 database connections
during busy windows.
...
relational_core, append_only_events, semi_structured_jsonb, and the workload patterns
are oltp_heavy, append_heavy, read_heavy, strong_tenant_locality, analytics_adjacent.
The growth horizon is concrete: in 6 months the team expects to absorb the next customer
or seasonal cycle, in 12 months they expect current hot tables to roughly follow 20
percent month-over-month growth unless product scope narrows, and in 24 months they
expect today's borderline decisions to become operational constraints. The known pending
decision is this: Citus is on the architecture board because the biggest tenant
dominates storage, but procurement requires portable managed PostgreSQL and no self-
managed cluster.

The scenario is not a textbook case. The largest tenant is huge but has cross-tenant
administrative workflows that do not always include a stable tenant distribution key.
