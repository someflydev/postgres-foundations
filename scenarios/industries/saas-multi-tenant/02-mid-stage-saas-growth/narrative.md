# Mid-stage SaaS growth with one large tenant

Mid-stage SaaS growth with one large tenant is a mid-stage pressure judgment brief from
a team of 26 engineers working in saas multi tenant. The product serves operators who
live in PostgreSQL-backed workflows every day: support staff, finance reviewers,
compliance analysts, and customer-facing teams who need answers while the business is
open. The current system runs on primary with read replicas with PostgreSQL as the
operational source of truth. Largest tables include accounts (6,400,000 rows),
activity_events (42,000,000 rows), invoices (9,000,000 rows). Peak traffic is roughly
2500 read queries per second, 420 writes per second, and 620 database connections during
busy windows.
...
The tenancy model is multi tenant schema per tenant. The data shape mix is
relational_core, semi_structured_jsonb, append_only_events, and the workload patterns
are oltp_heavy, read_heavy, analytics_adjacent, strong_tenant_locality. The growth
horizon is concrete: in 6 months the team expects to absorb the next customer or
seasonal cycle, in 12 months they expect current hot tables to roughly follow 16 percent
month-over-month growth unless product scope narrows, and in 24 months they expect
today's borderline decisions to become operational constraints. The known pending
decision is this: They are deciding whether the largest enterprise tenant should move to
its own schema and whether reporting should hit a replica or materialized rollups.

The scenario is not a textbook case. The largest tenant negotiates quarterly custom
data-retention terms, while smaller tenants expect shared release velocity. The team has
enough PostgreSQL skill to use indexes, constraints, RLS, replication, and managed
extensions, but not enough spare capacity to adopt every specialized tool at once. The
expected answer should separate core-first moves from extension candidates and should
say what evidence would change the recommendation. The reader should have to weigh
portability, tenant boundaries, audit expectations, and scale signals rather than simply
enabling a named extension.
