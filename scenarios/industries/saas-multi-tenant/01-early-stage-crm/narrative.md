# Early-stage CRM with shared-schema tenants

Early-stage CRM with shared-schema tenants is a small early conservative brief from a
team of 8 engineers working in saas multi tenant. The product serves operators who live
...
truth. Largest tables include accounts (320,000 rows), tenant_memberships (180,000
rows), audit_events (2,200,000 rows). Peak traffic is roughly 520 read queries per
second, 75 writes per second, and 140 database connections during busy windows.
...
semi_structured_jsonb, and the workload patterns are oltp_heavy, read_heavy,
strong_tenant_locality. The growth horizon is concrete: in 6 months the team expects to
absorb the next customer or seasonal cycle, in 12 months they expect current hot tables
to roughly follow 11 percent month-over-month growth unless product scope narrows, and
in 24 months they expect today's borderline decisions to become operational constraints.
The known pending decision is this: They are considering pgvector for account notes
because sales managers ask for semantic recall, but the team has not built a lexical
search baseline yet.

The scenario is not a textbook case. Two design partners use unusually broad custom
fields, so JSONB appears in hot screens even though most tenants remain simple. The team
