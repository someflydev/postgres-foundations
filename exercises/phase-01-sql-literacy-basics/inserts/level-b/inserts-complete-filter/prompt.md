# INSERT Rows: Complete Filter

Seed pack: `ecommerce`, phase `1`.

Write a one-table INSERT against `ecommerce.products` with an explicit column list. Use the scaffold in `starter.sql` if present. Your output must match the intended row set for `INSERT Rows: Complete Filter`.
