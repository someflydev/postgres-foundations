# Silent Wrong Rows: Complete Filter

Seed pack: `scheduling`, phase `1`.

Write a one-table SELECT against `scheduling.appointments`. Use the scaffold in `starter.sql` if present. Your output must match the intended row set for `Silent Wrong Rows: Complete Filter`.
