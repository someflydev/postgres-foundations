# WHERE Basics: Complete Order

Seed pack: `ecommerce`, phase `1`.

Write a one-table SELECT against `ecommerce.orders`. Use the scaffold in `starter.sql` if present. Your output must match the intended row set for `WHERE Basics: Complete Order`.
