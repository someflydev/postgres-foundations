# Shape of a Table: Complete Filter

Seed pack: `ecommerce`, phase `1`.

Write a one-table SELECT against `ecommerce.products`. Use the scaffold in `starter.sql` if present. Your output must match the intended row set for `Shape of a Table: Complete Filter`.
