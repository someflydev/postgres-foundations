# NULL and Missing Facts: Complete Order

Seed pack: `ecommerce`, phase `1`.

Write a one-table SELECT against `ecommerce.customers`. Use the scaffold in `starter.sql` if present. Your output must match the intended row set for `NULL and Missing Facts: Complete Order`.
