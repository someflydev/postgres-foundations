# UPDATE and DELETE Rows: Complete Filter

Seed pack: `scheduling`, phase `1`.

Write a one-table UPDATE or DELETE against `scheduling.appointments` with a protective `WHERE` clause. Use the scaffold in `starter.sql` if present. Your output must match the intended row set for `UPDATE and DELETE Rows: Complete Filter`.
