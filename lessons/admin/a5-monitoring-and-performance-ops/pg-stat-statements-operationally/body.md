...
Use pg_stat_statements as a workload ledger, not as a trivia table. Operators sort by total time to find where the database spends its week, by average time to find outlier pain, and by I/O counters to separate CPU work from reads that should be explained with buffers. The practical question is not whether a catalog view exists; it is what decision the view supports when the system is under pressure. A PostgreSQL operator should be able to collect evidence, name the risk, choose a bounded action, and describe how to verify the result. That habit keeps monitoring, performance triage, replication, and high-availability work connected to user-visible service behavior instead of isolated command memorization.

For this lesson the required vocabulary is: pg_stat_statements, top 5 by total time, average time, weekly triage rhythm. Use those terms in incident notes exactly enough that another operator could rerun the same evidence query and reach the same conclusion. If a metric or catalog row is ambiguous, say what additional observation would disambiguate it.
