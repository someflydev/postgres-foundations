...
PITR combines a base backup with archived WAL so recovery can replay to a timestamp, LSN, or named target. archive_command must be monitored as production infrastructure, and retention must cover both recovery goals and the time needed to discover an incident. In the administration track this is treated as production behavior, not a preference hidden in a generated file. The operator must be able to explain the boundary, show the catalog or configuration evidence, and choose the least surprising remediation. For this lesson the recurring vocabulary is PITR, archive_command, restore_command, retention. Each term is tied to a concrete failure mode: an app that cannot connect, a role that can connect too broadly, a session that holds resources indefinitely, a backup that cannot be restored, or a maintenance process that is not keeping pace with churn.
