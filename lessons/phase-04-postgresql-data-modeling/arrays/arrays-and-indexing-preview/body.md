...
Arrays and Indexing Preview belongs in Phase 4b because PostgreSQL-specific types should solve a
specific modeling problem, not decorate a schema. The concrete topic is recognizing that tags @> has an index shape without building it yet.
A good-fit example is noticing the future GIN index signal after the model is right. A bad-fit example is adding an index to rescue an array that should have been a child table. The learner should
...
Use recognizing that tags @> has an index shape without building it yet. In the good-fit design, the column or table expresses the business
