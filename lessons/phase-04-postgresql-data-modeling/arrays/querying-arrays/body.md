...
Querying Arrays belongs in Phase 4b because PostgreSQL-specific types should solve a
specific modeling problem, not decorate a schema. The concrete topic is finding products by tag and rolling tags back up for review.
A good-fit example is querying bounded tags directly from ecommerce.products. A bad-fit example is using array operators to avoid modeling facts that need rows. The learner should
...
Use finding products by tag and rolling tags back up for review. In the good-fit design, the column or table expresses the business
