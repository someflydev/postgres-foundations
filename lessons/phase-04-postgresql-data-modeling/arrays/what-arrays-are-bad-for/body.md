...
What Arrays Are Bad For belongs in Phase 4b because PostgreSQL-specific types should solve a
specific modeling problem, not decorate a schema. The concrete topic is user_roles text[] as a deliberately weak design.
A good-fit example is a child table for role grants when revocation and metadata matter. A bad-fit example is packing lifecycle-bearing role facts into roles text[]. The learner should
...
Use user_roles text[] as a deliberately weak design. In the good-fit design, the column or table expresses the business
