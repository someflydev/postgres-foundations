...
What Arrays Are Good For belongs in Phase 4b because PostgreSQL-specific types should solve a
specific modeling problem, not decorate a schema. The concrete topic is ARRAY['postgres','training'] tags on ecommerce.products.
A good-fit example is product tags that are small, bounded, and owned by the product row. A bad-fit example is roles, permissions, line items, or prices that need metadata and history. The learner should
...
Use ARRAY['postgres','training'] tags on ecommerce.products. In the good-fit design, the column or table expresses the business
