...
UUID v4 is random, which is simple and widely supported. UUID v7 is time-ordered, which can improve locality for write-heavy tables and make recent identifiers cluster more naturally. PostgreSQL 16 does not ship a built-in v7 generator, so this lesson treats v7 as a design discussion rather than a lab dependency. The `pg_uuidv7` extension is a later pointer when workload evidence justifies extension adoption; Phase 4a keeps `gen_random_uuid()` as the executable choice.
