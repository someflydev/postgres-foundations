...
`gen_random_uuid()` comes from `pgcrypto`, which is already enabled in the lab, and is the preferred core-first path here. `uuid-ossp` can also generate UUIDs, but it is a separate extension and should not be introduced just to do what `pgcrypto` already covers. UUIDs are a good fit for public references, distributed generation, and tenant/user identifiers that cross system boundaries. They are a weak default for dense internal join keys when no external exposure or distributed write path exists.
