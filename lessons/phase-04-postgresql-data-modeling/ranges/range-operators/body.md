...
Range Operators belongs in Phase 4b because PostgreSQL-specific types should solve a
specific modeling problem, not decorate a schema. The concrete topic is using &&, @>, <@, -|-, lower, and upper on scheduling slots.
A good-fit example is asking interval questions with PostgreSQL range operators. A bad-fit example is reimplementing interval arithmetic in application code. The learner should
...
Use using &&, @>, <@, -|-, lower, and upper on scheduling slots. In the good-fit design, the column or table expresses the business
