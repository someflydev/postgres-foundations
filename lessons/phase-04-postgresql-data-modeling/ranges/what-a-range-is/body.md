...
What a Range Is belongs in Phase 4b because PostgreSQL-specific types should solve a
specific modeling problem, not decorate a schema. The concrete topic is tstzrange appointment slots and visible bound notation.
A good-fit example is a single value representing a start, end, and bound convention. A bad-fit example is two loose columns whose overlap meaning lives only in app code. The learner should
...
Use tstzrange appointment slots and visible bound notation. In the good-fit design, the column or table expresses the business
