...
Exclusion Constraints belongs in Phase 4b because PostgreSQL-specific types should solve a
specific modeling problem, not decorate a schema. The concrete topic is EXCLUDE USING gist for professional appointment overlap.
A good-fit example is letting PostgreSQL reject conflicting appointments for the same professional. A bad-fit example is checking overlap only before insert in application code. The learner should
...
Use EXCLUDE USING gist for professional appointment overlap. In the good-fit design, the column or table expresses the business
