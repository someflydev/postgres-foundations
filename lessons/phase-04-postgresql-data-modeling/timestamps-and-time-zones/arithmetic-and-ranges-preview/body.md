...
`interval` values describe durations such as `30 minutes` or `7 days`. `now()` is stable inside the transaction, while `statement_timestamp()` names the start of the current statement. Phase 4a uses simple half-open filters such as `starts_at >= lower_bound AND starts_at < upper_bound` to preview range thinking without introducing range types yet. The learner should notice how often paired start/end comparisons are really asking for a range model that arrives in Phase 4b.
