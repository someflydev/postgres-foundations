...
Wall-clock storage is deliberate only when "9:00 AM in this local calendar" is the business fact before a concrete instant is known. A clinic template, a store opening rule, or a future recurring appointment pattern may need that shape. The caveat is that daylight-saving transitions, region changes, and later conversion to real appointments must be handled explicitly. Once a specific appointment is booked, `starts_at timestamptz` and the professional's `timezone` together preserve both the instant and the display rule.
