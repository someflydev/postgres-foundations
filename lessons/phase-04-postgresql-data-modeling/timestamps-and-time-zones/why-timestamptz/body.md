...
PostgreSQL stores `timestamptz` as an absolute instant and renders it through the session time zone on output. It does not store the original zone name with the value. That behavior is exactly why `timestamptz` is the default advice for event time: the order happened once, even if one user sees it in Chicago time and another sees it in UTC. A plain `timestamp` stores a calendar label without time-zone interpretation, which is useful only when the label itself is the fact.
