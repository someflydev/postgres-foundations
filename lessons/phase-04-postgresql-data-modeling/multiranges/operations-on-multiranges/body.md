...
Operations on Multiranges belongs in Phase 4b because PostgreSQL-specific types should solve a
specific modeling problem, not decorate a schema. The concrete topic is subtracting appointments and time off from a working calendar.
A good-fit example is using PostgreSQL operators to derive practical availability. A bad-fit example is splitting all calendar arithmetic into application loops. The learner should
...
Use subtracting appointments and time off from a working calendar. In the good-fit design, the column or table expresses the business
