...
What Multiranges Unlock belongs in Phase 4b because PostgreSQL-specific types should solve a
specific modeling problem, not decorate a schema. The concrete topic is professional working_hours as tstzmultirange.
A good-fit example is a calendar with multiple separated available windows. A bad-fit example is inventing comma-delimited times or scattered boolean flags. The learner should
...
Use professional working_hours as tstzmultirange. In the good-fit design, the column or table expresses the business
