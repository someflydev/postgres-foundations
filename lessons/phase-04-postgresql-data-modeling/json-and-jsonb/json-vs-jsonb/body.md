...
`json` preserves input text and reparses it when queried. `jsonb` stores a decomposed binary representation, removes insignificant whitespace, normalizes key order, and supports containment-style operators. That storage behavior is why this curriculum says to almost always prefer `jsonb` when PostgreSQL must query the value. The exception is rare: preserving the exact original JSON text as text-like evidence is a different requirement from modeling queryable data.
