...
The basic JSONB reading tools are intentionally small. `->` returns a JSON value, `->>` returns text, `@>` asks whether the left document contains the right document, and `jsonb_path_query` reads values through a JSON path expression. The lesson forbids `SELECT *` as a habit because JSONB can make row shape noisy. Select the key being inspected, give it a clear alias, and keep the relational grain visible.
