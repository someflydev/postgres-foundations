...
The migration out of JSONB is part of the modeling skill. Add the new typed column, backfill it from `metadata ->> 'key'`, inspect missing and misspelled keys, add `NOT NULL`, `CHECK`, `UNIQUE`, or a foreign key only after the data is clean, then update writers. Keeping the old JSONB key temporarily can be useful for audit or rollback, but the rewritten query should filter the column, not repeat the extraction.
