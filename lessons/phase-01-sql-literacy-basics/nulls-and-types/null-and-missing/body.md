...
```sql
SELECT email, full_name FROM ecommerce.customers WHERE full_name = 'Ada Lovelace';
```
...
A good explanation names the table, the intended rows, and the columns returned or changed. For a read query, say: this statement reads `ecommerce.customers`, keeps rows that meet the predicate, sorts them by the named column, and returns only the selected columns. For a change query, say: this statement changes only rows matching the `WHERE` clause, and `RETURNING` shows the affected rows so the result is auditable. If the explanation cannot point to concrete rows in the seed data, the SQL is probably ahead of the reasoning.
