...
What user-visible symptom caused this investigation? Which PostgreSQL core feature handles the requirement today, and where does it fail? What metric or result set would change if pgvector is the right tool? How often is the underlying data written, and who pays the write or collection overhead? Does the managed service support the required version and settings without superuser-only work? What evidence would prove that the change improved the workload instead of only moving cost elsewhere?
