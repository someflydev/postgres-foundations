...
What user-visible symptom caused this investigation? Which PostgreSQL core feature handles the requirement today, and where does it fail? What metric or result set would change if pg_trgm is the right tool? How often is the underlying data written, and who pays the write or collection overhead? Does the managed service support the required version and settings without superuser-only work? What evidence would prove that the change improved the workload instead of only moving cost elsewhere?
...
Use `docs/doctrine.md` for extension posture, `docs/search-playbook.md` for search boundaries, and the extension-track module docs for this lesson. Pair pg_trgm work with the Phase 7 indexing playbooks and the administration monitoring playbook when the change affects production operations.
