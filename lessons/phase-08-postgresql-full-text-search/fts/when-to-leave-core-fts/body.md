...
When the user intent is typo tolerance instead of lexical ranking, mark `pg_trgm` as a preview and keep the scope narrow. The deep dive is PROMPT_34, so Phase 8 only demonstrates the shape of the alternative:
