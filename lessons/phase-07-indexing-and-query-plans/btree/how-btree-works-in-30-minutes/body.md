# How B-tree Works in 30 Minutes

## Problem Framing

Phase 7a is where learners start treating query performance as an observable database behavior instead of folklore. This lesson focuses on B-tree ordering, equality, range predicates, NULLS FIRST/LAST, and INCLUDE columns. The doctrine stays the same: correctness comes first, and performance advice must be attached to a query pattern. An index is not a decoration on a column. It is an access path that is either useful for a predicate, ordering requirement, and projected column list, or it is extra maintenance work.
...
A B-tree stores keys in sorted order. Equality predicates jump to a narrow key region, range predicates walk from one boundary to another, and matching order can remove an explicit sort. PostgreSQL uses `NULLS FIRST` or `NULLS LAST` in the index definition when null placement matters for ordered retrieval. PostgreSQL's payload-column feature is `INCLUDE`; it is similar in intent to systems that call the feature storing columns, but included columns are not search keys.
...
Explain this lesson by saying the query pattern first: "For price and time ranges that can be entered through ordered keys, I expect this access path because..." Then name the index. A defensible answer includes the predicate, ordering requirement, expected selectivity, and maintenance cost. A weak answer names only a column.
