...
Phase 7a is where learners start treating query performance as an observable database behavior instead of folklore. This lesson focuses on sequential scans and narrow index lookups. The doctrine stays the same: correctness comes first, and performance advice must be attached to a query pattern. An index is not a decoration on a column. It is an access path that is either useful for a predicate, ordering requirement, and projected column list, or it is extra maintenance work.
...
The central contrast is not indexed equals good and sequential equals bad. A status count touches enough of the table that reading the table once can be cheaper than bouncing through an index and then visiting many heap pages. The recent-order lookup keeps very few rows and has an ordering requirement, so the index can enter at one customer and stop after the first matching row.
...
Explain this lesson by saying the query pattern first: "For broad status counts versus a recent-orders lookup for one customer, I expect this access path because..." Then name the index. A defensible answer includes the predicate, ordering requirement, expected selectivity, and maintenance cost. A weak answer names only a column.
