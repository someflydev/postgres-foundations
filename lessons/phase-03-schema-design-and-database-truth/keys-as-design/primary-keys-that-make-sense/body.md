# Primary Keys That Make Sense

## Problem Framing

A primary key is a design choice, not decoration. A surrogate bigint identity is often the right first tool because it is narrow, stable, and generated by PostgreSQL. A natural key can be right when the business identifier is truly durable, but emails, names, and SKUs often change or carry exceptions. UUIDs matter later, especially across distributed boundaries, but this phase keeps the center on bigint identity and explicit uniqueness for business identifiers.
...
Primary Keys That Make Sense is taught through ordinary PostgreSQL DDL: CREATE TABLE, ALTER TABLE, NOT NULL, UNIQUE, CHECK, DEFAULT, and FOREIGN KEY. The important habit is naming the invariant before writing syntax. If the invariant cannot be stated in a sentence, the learner is probably copying a pattern instead of designing a rule. PostgreSQL core features are enough here; JSONB, arrays, partitioning, and tuning indexes stay out of scope.
